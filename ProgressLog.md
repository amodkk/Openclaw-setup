...
|------|----------------------|----------------------------------|
| 04/26/2026 7:25 PM CST | **Gateway container was recreated (why `gh` auth disappeared)** | `openclaw-openclaw-gateway-1` was recreated today as part of the `docker compose up -d` run after updating `docker-compose.yml` to expose the Vite port. Since `gh` auth lived inside the container filesystem (e.g. `/home/node/.config/gh/hosts.yml`), it was wiped when the container was replaced. |
| 04/26/2026 | **Vite dev port exposed from OpenClaw container**<br>Fixed Docker networking so a Vite/Svelte dev server started inside the OpenClaw container is reachable from the host browser. | **Vite/Svelte dev server in Docker:** Root cause was the usual container networking mismatch: Vite binds to `localhost` by default (container-only), and Docker must publish the port to the host. Fixes applied:<br>• Updated `C:\Users\amodk\Documents\repos\openclaw\docker-compose.yml` to publish `5173:5173` on the `openclaw-gateway` service, then recreated the containers with `docker compose up -d`.<br>• Verified port publishing with `docker ps` showing `0.0.0.0:5173->5173/tcp`.<br>**What to run inside the container:** `npm run dev -- --host 0.0.0.0 --port 5173 --strictPort` and browse from Windows at `http://127.0.0.1:5173`. |
